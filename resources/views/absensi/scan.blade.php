...
<div class="bg-white rounded-lg shadow max-w-2xl mx-auto">
    <div class="p-6 border-b">
        <h1 class="text-2xl font-bold">Scan QR Code Absensi</h1>
    </div>
    ...
    <div class="p-6 text-center">
        <div class="mb-4">
            ...
            <select id="jadwal_id" class="w-full border rounded-lg px-4 py-2">
                ...
                <option value="{{ $j->id }}">
                    {{ $j->nama_mk }} - {{ $j->hari }} {{ substr($j->jam_mulai,0,5) }} ({{ $j->nama_dosen }})
                </option>
                @endforeach
            </select>
        </div>
        ...
        <button id="generate-btn" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg">
...
document.getElementById('generate-btn').addEventListener('click', function() {
    ...
    window.location.href = '/absensi-generate-qr/' + jadwalId;
});
</script>
@endsection