@extends('layouts.app')

@section('title', 'Detail Jurusan')

@section('content')
<div class="bg-white rounded-lg shadow">
    <div class="p-6 border-b flex justify-between items-center">
        <h1 class="text-2xl font-bold">Detail Jurusan</h1>
        <div class="flex gap-2">
            <a href="{{ route('jurusan.edit', $jurusan->id) }}" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg">
                <i class="fas fa-edit mr-2"></i>Edit
            </a>
            <a href="{{ route('jurusan.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
                <i class="fas fa-arrow-left mr-2"></i>Kembali
            </a>
        </div>
    </div>
    
    <div class="p-6">
        <div class="grid md:grid-cols-2 gap-6 mb-6">
            <div class="bg-gray-50 p-4 rounded-lg">
                <h3 class="font-semibold text-gray-700 mb-2">Informasi Jurusan</h3>
                <table class="w-full">
                    <tr>
                        <td class="py-2 text-gray-600 w-32">Kode Jurusan</td>
                        <td class="py-2">: {{ $jurusan->kode_jurusan }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-600">Nama Jurusan</td>
                        <td class="py-2">: {{ $jurusan->nama_jurusan }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-600">Deskripsi</td>
                        <td class="py-2">: {{ $jurusan->deskripsi ?? '-' }}</td>
                    </tr>
                </table>
            </div>
            
            <div class="bg-gray-50 p-4 rounded-lg">
                <h3 class="font-semibold text-gray-700 mb-2">Statistik</h3>
                <table class="w-full">
                    <tr>
                        <td class="py-2 text-gray-600 w-32">Total Mahasiswa</td>
                        <td class="py-2">: {{ $statistik['total_mahasiswa'] ?? 0 }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-600">Total Mata Kuliah</td>
                        <td class="py-2">: {{ $statistik['total_matakuliah'] ?? 0 }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-600">Total Dosen</td>
                        <td class="py-2">: {{ $statistik['total_dosen'] ?? 0 }}</td>
                    </tr>
                </table>
            </div>
        </div>
        
        <!-- Daftar Mahasiswa -->
        <div class="mb-6">
            <h2 class="text-lg font-semibold mb-3">Daftar Mahasiswa (10 terbaru)</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full border">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-2 text-left">NIM</th>
                            <th class="px-4 py-2 text-left">Nama</th>
                            <th class="px-4 py-2 text-left">Angkatan</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($mahasiswas as $mhs)
                        <tr>
                            <td class="px-4 py-2">{{ $mhs->nim }}</td>
                            <td class="px-4 py-2">{{ $mhs->nama_mahasiswa }}</td>
                            <td class="px-4 py-2">{{ $mhs->angkatan }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="3" class="px-4 py-2 text-center">Tidak ada data</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
        
        <!-- Daftar Mata Kuliah -->
        <div>
            <h2 class="text-lg font-semibold mb-3">Daftar Mata Kuliah</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full border">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-2 text-left">Kode MK</th>
                            <th class="px-4 py-2 text-left">Nama Mata Kuliah</th>
                            <th class="px-4 py-2 text-left">SKS</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($matakuliahs as $mk)
                        <tr>
                            <td class="px-4 py-2">{{ $mk->kode_mk }}</td>
                            <td class="px-4 py-2">{{ $mk->nama_mk }}</td>
                            <td class="px-4 py-2">{{ $mk->sks }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="3" class="px-4 py-2 text-center">Tidak ada data</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
@endsection